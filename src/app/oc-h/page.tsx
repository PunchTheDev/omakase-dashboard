...
// OC-H — the harness competition: main baseline, delta tiers, pinned router.
...
export default function OcH() {
  ...
  const pin = routerPin();
  const entries = frontier("oc-harness").slice().reverse();
  const tiers = cfg?.delta_tiers ?? {};

  return (
    <div>
      <h1 className="text-lg font-semibold">OC-H · orchestration harness</h1>
      <p className="mt-1 max-w-2xl text-sm" style={{ color: "var(--ink-2)" }}>
        ...
        <Link href="/docs/miner-agent-oc-h" className="underline">MINER-AGENT.md</Link> is the full contract.
      </p>
      ...
      <SectionTitle hint="per-suite weaknesses live on OC-R's gap analysis">Where gains live</SectionTitle>
      <div className="card px-5 py-4 text-sm leading-relaxed" style={{ color: "var(--ink-2)" }}>
        ...
        suggestion) · the <Link href="/oc-r" className="underline">gap analysis</Link> lists the champion&apos;s
